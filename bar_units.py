...
filterOptions = {
    ...
    "metalmake": (
        [["armorcore", "is", True], ["energymake", ">", 5], ["type", "is", "building"]],
...
def main(filters, selection):
    ...
    try:
        units = {}
        for unit in unitTupleList:
            # Preprocess the unit
            id, value = process.preprocess(unit)
            # Extend the dictionary with the processed values
            units[id] = value
    except Exception as e:
        print(f"Problem encountered during processing {unit[0]}, ignoring")

    with open("generated/unitsExtended.json", "w", encoding="utf-8") as f:
        json.dump(units, f, ensure_ascii=False, indent=4)

    output.write(filters=filters, select=selection, data=units)
